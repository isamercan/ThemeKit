...
| Planned enum renames (ADR-0009 D5) | `ThemeButtonStyle`, `BadgeStyle`, `CalloutStyle`, `RadioButtonStyle` → names that say what they pick (preset, tone, surface, indicator); each old spelling stays as a deprecated `typealias` through 2.x; the `…Style` names are free for the chrome-style protocols only once those aliases go | 2.0 (renamed); aliases removed at the next major |
