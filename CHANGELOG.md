...
### Fixed
- **Dark-mode preview coverage — Atoms** (audit P2, preview-only) — ~44 atom `#Preview`s adopt the `PreviewMatrix` helper, so each now renders its states across **light + dark** columns (previously only a handful had any dark case). No component or API change. Molecules/Organisms follow the same adoption.
