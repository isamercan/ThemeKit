...
### Changed (2.0 — public style system)
...
### Fixed
- **Dark-mode preview coverage — Atoms + Molecules** (audit P2, preview-only) — ~44 atom and ~86 molecule `#Preview`s adopt the `PreviewMatrix` helper, so each now renders its states across **light + dark** columns (previously only a handful had any dark case). No component or API change. Organisms follow the same adoption.
