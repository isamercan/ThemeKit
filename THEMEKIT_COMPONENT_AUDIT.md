...
> **✅ SHIPPED (PR-2a `fix/audit-p1-a11y-i18n`, API-safe, 53 files).** Locale capture, `String(themeKit:)` wrapping, and icon-button/interactive-row accessibility done across the components below (build + 284 tests green, `diagnose-api-breaking-changes` clean). Bonus: fixed a real `LocalizedStringKey`→consumer-bundle mis-resolution in `GuestSelector`/`FlightRoute`/`FlightCard` (now `String(themeKit:)`), and a Turkish brand leak (`SheetHeader` preview `"UcuzaBilet"` → `"Filters"`). **Follow-ups (not P1-mechanical):** `SeatCell` a11y strings + locale (Atoms; rides the SeatCell major-bump rework), `FlightStatus.label` raw literals (`FlightModels.swift`), a `RemoteImage.altText(_:)` modifier (additive API).
- [x] **`@Environment(\.locale)` capture** in the price/number components: PriceTag, PointsBadge, RollingNumber, GaugeView, Rating, RatingSummary, PriceBreakdown, PriceHistogram, PriceTrendChart, InstallmentPicker, InstallmentSelector, FlightResultRow, FlightCard, RoomCard, SeatMap, LoyaltyCard, AgentPriceRow, FareSummary, DatePriceStrip.
- [x] **Wrapped UI/a11y strings** in `String(themeKit:)`: BoardingPass, Coupon, FilterBar, FlightResultRow, FlightTicketCard, CountdownTimer, FareFeatureRow, ShareButton, SwapButton, QRCode, StepperRow, DatePriceStrip, AmenityGrid, CurrencyPicker, FileInput, SearchBar, Select, RecentSearchRow, SearchField, Barcode, ReviewCard, SheetHeader, FareSummary, SortSummaryBar, PassengerRow, GuestSelector, FlightRoute, Mentions, PriceHistogram, PriceTrendChart, InstallmentPicker. *(FlightStatusBadge → its labels live in `FlightStatus.label`, follow-up.)*
- [x] **Icon-only button `accessibilityLabel`s:** AlertToast, FilterGroup, QuantityStepper (+value), SearchBar, DatePriceStrip/PriceTrendChart (chevrons), SegmentedTabBar, Upload, ReviewCard (photos), CountBadge/Indicator (`.accessibilityHidden`), Barcode, RemoteImage (alt-text), SwapButton, Coupon, FlightResultRow/FlightCard/FlightTicketCard (favourite/bookmark, state-aware).
- [x] **Interactive rows a11y trait:** DataTable (`.isButton`/`.isSelected`), AgentPriceRow, PriceBreakdown (`.combine`), RoomCard/SegmentedTabBar (`.isSelected`).
- [x] **✅ SHIPPED (PR-2b `fix/audit-p1-rtl`, API-safe) — Adjustable/paging VoiceOver:** Diff (`accessibilityAdjustableAction` + label/value), PagingCarousel (paging adjustable + page value), Splitter (handle label + adjustable).
- [x] **✅ SHIPPED (PR-2b) — RTL geometry** (mirror `.offset`/`.position`/drag-math, flip `Path`s, thread `layoutDirection` into custom `Layout`s): Slider, RangeSlider, Timeline (rail), Steps (connector), PagingCarousel, Diff (divider+drag), Masonry (`MasonryLayout`), Flex (`FlexLayout`), RollingNumber (digit HStack pinned LTR), Indicator (nudge), Transfer (`chevron.forward/backward`), Tooltip (arrow flip + offset), LoyaltyCard (Canvas draws from trailing). Each changed file gained an RTL `#Preview`. **Follow-up:** `FlowLayout` (public shared `Layout`, used by Flex.wrap + 4 others) needs `layoutDirection` threaded through its public API — deferred to avoid an API change here.

## 🟡 P2 — systemic sweeps & enforcement

- [~] **Dark/themed preview coverage — standardizing on `PreviewMatrix`** (light + dark columns per `#Preview`). **Atoms + Molecules + Organisms done** (~44 atom + ~86 molecule + ~63 organism → PreviewMatrix; support files, native-`.sheet` BottomSheet, and interactive-only support previews are the documented remainder). The layer sweep is complete. Preview-only (no runtime/API impact).
...
- [x] **CI grep gates (prevent regression):** `scripts/check-neutrality.sh` **hard-fails** on `"etstur"` / Turkish-language tokens and any hardcoded `"TRY"` default (excluding the `CurrencyPicker` catalog); wired into `scripts/ci.sh` (pre-push) and a `neutrality` job in `.github/workflows/ci.yml`. Raw `Color.white/.black/hex` and in-component `URLSession` are printed as **advisory** counts (not blocking yet — they gate the P0.2/P0.3 sweeps). *(Shipped.)*
