...
enum ComponentRegistry {
    static let all: [ComponentEntry] = [
        ...
        .knob("TrendChip", .atoms, demo: TrendChipDemo(), usage: #"TrendChip(.up("+12%"))   // .down("-8%").positiveIsUp(false) → a fall reads green"#, isNew: true),
        ...
        .knob("Watermark", .atoms, demo: WatermarkDemo(), usage: #"ReportCard().watermark("CONFIDENTIAL", rotation: .degrees(-22))"#),
        ...
        .knob("ColorSwatchPicker", .molecules, demo: ColorSwatchPickerDemo(), usage: #"ColorSwatchPicker(palette, selection: $brand).columns(6).swatchShape(.circle)"#, isNew: true),
        .knob("ColorSlider", .molecules, demo: ColorSliderDemo(), usage: #"ColorSlider(.hue, color: $hsba)   // .saturation/.brightness/.alpha · .trackHeight(.compact)"#, isNew: true),
        ...
        .knob("ButtonGroup", .molecules, demo: ButtonGroupDemo(), usage: #"ButtonGroup(.horizontal) { SecondaryButton("Cancel"){}; PrimaryButton("Confirm"){} }.size(.large).width(.fill).dividers()"#),
        ...
        .knob("Cascader", .molecules, demo: CascaderDemo(), usage: #"Cascader(regions, selection: $path).placeholder("Region")"#),
        .knob("Transfer", .molecules, demo: TransferDemo(), usage: #"Transfer(items, target: $enabled).titles("Available", "Enabled")"#),
        .knob("Mentions", .molecules, demo: MentionsDemo(), usage: #"Mentions(text: $note, options: teammates).placeholder("Write a note…")"#),
        .knob("Masonry", .molecules, demo: MasonryDemo(), usage: #"Masonry { ForEach(photos) { Card($0) } }.columns(2).spacing(.sm)"#),
        .knob("Tree", .molecules, demo: TreeViewDemo(), usage: #"TreeView(nodes, selection: $checked).checkable()"#),
        .knob("Grid", .molecules, demo: ColumnsGridDemo(), usage: #"ColumnsGrid { ForEach(items) { Card($0) } }.columns(3).gutter(.md)"#),
        ...
        .knob("Calendar", .molecules, demo: CalendarDemo(), usage: #"CalendarView(selection: $date)   // .yearPicker() → tap header to jump months/years"#),
        .knob("Calendar Year Picker", .molecules, demo: CalendarYearPickerDemo(), usage: #"CalendarYearPicker(selection: $year).range(2000...2030).accent(.success)"#, isNew: true),
        ...
        .knob("LineChart", .molecules, demo: LineChartDemo(), usage: #"LineChart([ChartSeries("2026", points)], selection: $x).curved().showsPoints()"#, isNew: true),
        .knob("AreaChart", .molecules, demo: AreaChartDemo(), usage: #"AreaChart(series).stacked().curved()   // washes under 2pt outlines"#, isNew: true),
        .knob("BarChart", .molecules, demo: BarChartDemo(), usage: #"BarChart(series).mode(.grouped)   // .stacked · rounded caps · scrub readout"#, isNew: true),
        .knob("DonutChart", .molecules, demo: DonutChartDemo(), usage: ##"DonutChart(slices).innerRadius(.thin).label { Text("128k") }"##, isNew: true),
        .knob("HoverCard", .molecules, demo: HoverCardDemo(), usage: #"avatar.hoverCard(edge: .bottom) { ProfilePreview() }   // long-press / hover"#, isNew: true),
        .knob("EmojiReactionButton", .molecules, demo: EmojiReactionButtonDemo(), usage: ##"EmojiReactionButton("👍", count: 12, isReacted: $reacted)"##, isNew: true),
        .knob("ContextMenu", .molecules, demo: ThemeContextMenuDemo(), usage: ##"row.themeContextMenu([MenuAction("Delete", systemImage: "trash", role: .destructive) { … }])"##, isNew: true),
        .knob("Table Cells", .molecules, demo: TableCellsDemo(), usage: ##"TableToggleCell(isOn: $on, label: "Active")   // + Select / Slider / Color cells for DataTable"##, isNew: true),
        ...
        .knob("Popover", .organisms, demo: PopoverDemo(), usage: ##"trigger.themePopover(isPresented: $show, edge: .bottom, showsArrow: true) { FilterPanel() }"##, isNew: true),
        .knob("CommandPalette", .organisms, demo: CommandPaletteDemo(), usage: ##"root.commandPalette(isPresented: $show, sections: [CommandSection("Actions", items: […])])"##, isNew: true),
        .knob("ActionBar", .organisms, demo: ActionBarDemo(), usage: ##"list.actionBar(selection: $selected, actions: [ActionBarAction("Delete", systemImage: "trash", role: .destructive) { … }])"##, isNew: true),
        .knob("Agenda", .organisms, demo: AgendaDemo(), usage: #"Agenda(events).showsDayHeaders()   // date-grouped schedule with a time rail"#, isNew: true),
        .knob("ColorPickerPanel", .organisms, demo: ColorPickerPanelDemo(), usage: #"ColorPickerPanel(color: $hsba).swatches(palette).showsHexField()"#, isNew: true),
        ...
        .knob("AlertDialog", .organisms, demo: AlertDialogDemo(), usage: ##"view.alertDialog(isPresented: $show) { AlertDialog("Delete product", message: "…").icon("trash").tone(.error).primaryAction("Delete") { … }.secondaryAction("Cancel") { … }.closable { … }.footerLayout(.auto) }"##, isNew: true),
        ...
        .knob("FlightListItem", .organisms, demo: FlightListItemDemo(), usage: ##"FlightListItem(airline: "Skyline Air", from: "IST", to: "LHR", departure: dep, arrival: arr).price(214, currencyCode: "USD", caption: "from").badge("Best").onSelect { }.flightListItemStyle(.timeline)   // 9 styles: .compact/.timeline/.fareBoard/.deal/.ticket/.journey/.slices/.timetable/.tray"##),
        ...
        .knob("AirportPicker", .organisms, demo: AirportPickerDemo(), usage: ##"AirportPicker(selection: $airport, suggestions: results).onQueryChange { search($0) }.recent(recents).presentation(.sheet)   // ThemeKitTravel"##, isNew: true),
        .knob("TripSearchCard", .organisms, demo: TripSearchCardDemo(), usage: ##"TripSearchCard(draft: $trip, onSearch: submit).airports(suggestions: results, recent: recents).variant(.hero)   // ThemeKitTravel"##, isNew: true),
        .knob("SavedCardsList", .organisms, demo: SavedCardsListDemo(), usage: ##"SavedCardsList(cards, selection: $cardID).flagsExpired().onDelete { remove($0) }   // ThemeKitTravel"##, isNew: true),
        .knob("TransportCrossSellCard", .organisms, demo: TransportCrossSellCardDemo(), usage: ##"TransportCrossSellCard(.bus, from: "Riverton", to: "Lakeside").price(19).duration("6h 30m").onSelect { }   // ThemeKitTravel"##, isNew: true),
        .knob("FlightTracker", .organisms, demo: FlightTrackerDemo(), usage: ##"FlightTracker(info).progress(0.62).details([("Gate", "A12")]).showsTimeline()   // ThemeKitTravel"##, isNew: true),
        .knob("CheckInFlow", .organisms, demo: CheckInFlowDemo(), usage: ##"CheckInFlow(steps: steps, selection: $step) { page($0) }.canAdvance { done($0) }.onComplete { }   // ThemeKitTravel"##, isNew: true),
